...
if [[ -n "${EXISTING_ORACLE_JOB_ID}" ]]; then
    ...
    ORACLE_JOB=$(normalize_job_id "$(
        sbatch --parsable \
            ...
            --export="ALL,PROJECT_DIR=${REPO_ROOT},PYTHON_BIN=${PYTHON_BIN},CLIP_PARQUET=${CLIP_PARQUET},CANDIDATE_DIR=${CANDIDATE_DIR},OUTPUT_DIR=${ORACLE_DIR}" \
            "${REPO_ROOT}/slurm/build_lidar_world_oracle.sh"
    )")
fi

COMMON_EXPORT="ALL,BENCHMARK_REPO_ROOT=${REPO_ROOT},BENCHMARK_PYTHON=${PYTHON_BIN},BENCHMARK_RUN_DIR=${RUN_DIR},BENCHMARK_CANDIDATE_DIR=${CANDIDATE_DIR},BENCHMARK_ORACLE_DIR=${ORACLE_DIR},BENCHMARK_CLIP_PARQUET=${CLIP_PARQUET},BENCHMARK_EXPECTED_SHARDS=${EXPECTED_SHARDS},BENCHMARK_EXPECTED_ROWS=${EXPECTED_ROWS},BENCHMARK_EXPECTED_CANDIDATES=${EXPECTED_CANDIDATES},BENCHMARK_EPOCHS=${EPOCHS}"
